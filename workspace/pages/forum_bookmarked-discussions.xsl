...
<xsl:template match="data">
	...
	<table id="discussions" cellpadding="0" cellspacing="0">
		...
		<tbody>
			<td colspan="4" class="notification">This is a filtered view of discussions you have bookmarked. <a href="{$root}/forum/">Return to all discussions</a>.</td>
			<xsl:apply-templates select="forum-discussions-bookmarked/entry"/>
			<xsl:apply-templates select="forum-discussions-bookmarked/error"/>
		</tbody>
	</table>
	<xsl:call-template name="pagination">
	    <xsl:with-param name="pagination-element" select="forum-discussions-bookmarked/pagination" />
...
<xsl:template match="forum-discussions-bookmarked/entry">

	<!-- -1 to offset the first comment since it's a 'discussion' -->
	<xsl:variable name="id" select="@id" /><!-- This sucks. Please fix :( -->
	<xsl:variable name="total-replies" select="count(//forum-discussion-comment-count/parent-id[@link-id = $id]//entry) - 1"/>
	...
	<tr id="discussion-{@id}" class="{$status}">
		...
		<td class="last-post">
			<a href="{$root}/forum/discussions/{@id}/{$last-comment-page}/#position-{$total-replies + 1}">
